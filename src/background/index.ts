...
import { getSettings, saveSettings, getContextData, addLogEntry } from '../utils/storageUtils';
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.action) {
      ...
      case ACTIONS.FILL_FORMS:
        ...
        handleFillForms(sender.tab?.id, sendResponse);
        break;
...
const handleFillForms = async (
  ...
  try {
    ...
    if (!formData.success || !formData.elements || formData.elements.length === 0) {
 ...
 */
const requestFormData = async (tabId: number): Promise<{
  success: boolean;
  ...
  return new Promise((resolve) => {
    chrome.tabs.sendMessage(tabId, { action: ACTIONS.GET_FORM_DATA }, (response) => {
