 ...
 */
export const saveContextData = async (format: ContextFormat, data: string): Promise<void> => {
 ...
 */
export const getContextData = async (): Promise<string> => {
  const settings = await getSettings();
  return settings.contextData[settings.selectedFormat];
};
 ...
 */
export const addLogEntry = async (entry: Omit<LogEntry, 'timestamp'>): Promise<void> => {
